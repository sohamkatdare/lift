...
function addPlanet(mapTexture, size, detail, link = false) {
  const texture = new THREE.TextureLoader().load((link ? '' : './assets/') + mapTexture);
...
function addNormalPlanet(mapTexture, size, detail, normalMapTexture, link = false, normalLink = false) {
  const texture = new THREE.TextureLoader().load((link ? '' : './assets/') + mapTexture)
  const normalTexture = new THREE.TextureLoader().load((normalLink ? '' : './assets/') + normalMapTexture)
  const geometry = new THREE.DodecahedronGeometry(size, detail, detail);
...
const moon = addNormalPlanet('https://va3c.github.io/three.js/examples/textures/land_ocean_ice_cloud_2048.jpg', 3, 32, '2k_earth_normal.jpeg', true);
...
const uranus = addPlanet('https://static.wikia.nocookie.net/planet-texture-maps/images/c/c2/Dh_uranus_texture.png', 4, 32, true);
